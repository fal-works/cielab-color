...
/**
 * Clips RGB factor to the valid range (0 - 1) and converts it to sRGB value (0 - 255).
 * @param factor - Factor of either red, green or blue.
 * @returns SRGB value.
 */
function rgbFactorToSrgbValue(factor) {
...
function cielabColor(lValue, aValue, bValue, alphaValue) {
    ...
    const srgbValue = assignArray(temporalArray4, rgbFactorToSrgbValue(rgbFactor[0]), rgbFactorToSrgbValue(rgbFactor[1]), rgbFactorToSrgbValue(rgbFactor[2]));
    return alphaValue ? [
        srgbValue[0],
        srgbValue[1],
        srgbValue[2],
